...
import { authenticatedGit, repoCloneUrl } from '../utils/git-util';
...
async function updateRepoCache({ slug, accessToken }: InitRepoOptions) {
  ...
  const git = authenticatedGit(cacheDir, accessToken);
  if (!(await git.checkIsRepo(CheckRepoActions.BARE))) {
    ...
    await fs.promises.mkdir(cacheDir, { recursive: true });
    await git.clone(repoCloneUrl(slug), '.', ['--bare']);
  } else {
    // Ensure a previously-cached origin never retains an embedded credential.
    await git.remote(['set-url', 'origin', repoCloneUrl(slug)]);
  }
  await git.fetch();
